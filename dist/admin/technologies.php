...
if (isset($_POST['technologyAdd'])) {
  ...
  if (isset($_POST['technology-name']) && isset($_FILES['technology-image'])) {
    ...
    if ($res) {
      ...
      // VERIF LA TAILLE DE L'IMAGE
      // VERIF LA COULEUR DE L'IMAGE
      // SINON REMPLACER ?
        ...
        if ($file_error === 0) {
          if ($file_size < 1000000) {
            ...
            if (move_uploaded_file($file_tmp, $file_destination)) {
              $_SESSION['success'][] = 'Technology image successfully added';
            } else {
    ...
    <div class="p-6 border border-buttonColor-borderColor-normal bg-bodyBack rounded-xl z-20 w-full flex flex-col gap-2 h-fit">
      ...
      <ul class="mt-4 w-full">
        <?php foreach ($technologies as $technology) { ?>
          <li class="flex justify-between gap-6 items-center py-2 border-b border-buttonColor-borderColor-normal">
            ...
            <a href="delete_technology.php?id=<?= $technology['id'] ?>" class="text-base font-medium py-1 px-4 border border-buttonColor-borderColor-normal rounded-md text-accentColor-100">Delete</a>
          </li>
        <?php } ?>
      </ul>
    </div>
